...
export default function Navigation({ currentPage }: NavigationProps) {
  ...
  const [isMobileSeoToolsOpen, setIsMobileSeoToolsOpen] = useState(false)
      ...
      <nav className="hidden md:flex items-center gap-1">
          ...
          <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-80 bg-white border border-neutral-100 rounded-2xl shadow-xl shadow-neutral-100/80 transition-all duration-200 ${isServicesOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
            ...
            <div className="p-3">
                ...
                <Link key={service.no} href={service.href} className="flex items-center gap-3 p-3 rounded-xl hover:bg-neutral-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-neutral-900 group-hover:text-neutral-700 transition-colors truncate">{service.title}</p>
              ...
              <div className="border-t border-neutral-100 mt-2 pt-2">
                <Link href="/services" className="flex items-center justify-between p-3 rounded-xl hover:bg-neutral-50 transition-colors group">
                  <div>
        ...
        <div className="relative" onMouseEnter={() => setIsSeoToolsOpen(true)} onMouseLeave={() => setIsSeoToolsOpen(false)}>
          <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-80 bg-white border border-neutral-100 rounded-2xl shadow-xl shadow-neutral-100/80 transition-all duration-200 ${isSeoToolsOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
            ...
            <div className="p-3">
              {seoTools.map((tool) => (
                <Link key={tool.no} href={tool.href} className="flex items-center gap-3 p-3 rounded-xl hover:bg-neutral-50 transition-colors group">
                  <div className="w-8 h-8 rounded-full border border-neutral-100 flex items-center justify-center text-neutral-400 group-hover:border-neutral-300 transition-colors flex-shrink-0">
                    {tool.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-neutral-900 group-hover:text-neutral-700 transition-colors truncate">{tool.title}</p>
                      <span className="text-[10px] font-mono text-neutral-300 flex-shrink-0">{tool.no}</span>
                    </div>
                    <p className="text-[11px] text-neutral-400 mt-0.5 truncate">{tool.description}</p>
                  </div>
                </Link>
              ))}
              <div className="border-t border-neutral-100 mt-2 pt-2">
                <Link href="/seo-tools" className="flex items-center justify-between p-3 rounded-xl hover:bg-neutral-50 transition-colors group">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">View All SEO Tools</p>
                    <p className="text-[11px] text-neutral-400 mt-0.5">Free SEO utilities</p>
                  </div>
                  <div className="w-7 h-7 rounded-full border border-neutral-100 flex items-center justify-center group-hover:border-neutral-900 group-hover:bg-neutral-900 transition-all">
                    <ArrowUpRight className="w-3 h-3 text-neutral-300 group-hover:text-white transition-colors" />
                  </div>
        ...
        <div className="px-5 py-6 space-y-1">
          ...
          <div>
            <button onClick={() => setIsMobileServicesOpen(!isMobileServicesOpen)} className={`flex items-center justify-between w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isServicePage ? 'bg-neutral-100 text-neutral-900' : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'}`}>
              Services
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isMobileServicesOpen ? 'rotate-180' : ''}`} />
            </button>
            <div className={`overflow-hidden transition-all duration-300 ${isMobileServicesOpen ? 'max-h-96 mt-1' : 'max-h-0'}`}>
              <div className="pl-3 space-y-0.5 border-l border-neutral-100 ml-3">
                ...
                <Link href="/services" onClick={closeMobileMenu} className="flex items-center gap-2 px-3 py-2.5 text-sm font-medium text-neutral-900 hover:bg-neutral-50 rounded-xl transition-colors">
          ...
          <div>
            <button onClick={() => setIsMobileSeoToolsOpen(!isMobileSeoToolsOpen)} className={`flex items-center justify-between w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isSeoToolsPage ? 'bg-neutral-100 text-neutral-900' : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'}`}>
              SEO Tools
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isMobileSeoToolsOpen ? 'rotate-180' : ''}`} />
            </button>
            <div className={`overflow-hidden transition-all duration-300 ${isMobileSeoToolsOpen ? 'max-h-96 mt-1' : 'max-h-0'}`}>
              <div className="pl-3 space-y-0.5 border-l border-neutral-100 ml-3">
                {seoTools.map((tool) => (
                  <Link key={tool.no} href={tool.href} onClick={closeMobileMenu} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-neutral-50 transition-colors group">
                    <div className="w-7 h-7 rounded-full border border-neutral-100 flex items-center justify-center text-neutral-400 flex-shrink-0">{tool.icon}</div>
                    <div>
                      <p className="text-sm font-medium text-neutral-900">{tool.title}</p>
                      <p className="text-[11px] text-neutral-400">{tool.description}</p>
                    </div>
                  </Link>
                ))}
                <Link href="/seo-tools" onClick={closeMobileMenu} className="flex items-center gap-2 px-3 py-2.5 text-sm font-medium text-neutral-900 hover:bg-neutral-50 rounded-xl transition-colors">
                  View All SEO Tools <ArrowUpRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </div>
          </div>
