  ...
  const seoTools = [
    {
      ...
      description: 'Audit title, description, OG & Twitter tags',
      icon: <ScanSearch className="w-4 h-4" />,
      href: '/seo-tools/open-graph-checker',
      no: '01',
    },
    {
      title: 'Title Tag Checker',
      description: 'Check on-page SEO issues and quick wins',
      icon: <ShieldCheck className="w-4 h-4" />,
      href: '/seo-tools/title-tag-checker',
      no: '02',
    },
    {
      title: 'Meta Description Checker',
      description: 'Check meta descriptions',
      icon: <Layers3 className="w-4 h-4" />,
      href: '/seo-tools/meta-description-checker',
      no: '03',
    },
  ]

  const isServicePage =
    (currentPage ?? '').startsWith('services') || currentPage === 'services'

  const isSeoToolsPage =
    (currentPage ?? '').startsWith('seo-tools') || currentPage === 'seo-tools'
    ...
    <>
      <nav className="hidden md:flex items-center gap-1">
        ...
        >
          ...
          <div
            className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-80 bg-white border border-neutral-100 rounded-2xl shadow-xl shadow-neutral-100/80 transition-all duration-200 ${
              isServicesOpen
        ...
        <div
          className="relative"
          onMouseEnter={() => setIsSeoToolsOpen(true)}
          onMouseLeave={() => setIsSeoToolsOpen(false)}
        >
          <button
            type="button"
            className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
      ...
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-white border-b border-neutral-100 shadow-xl shadow-neutral-100/50 transition-all duration-300 overflow-hidden ${
      ...
      >
        <div className="px-5 py-6 space-y-1">
          ...
          <div>
            <button
              onClick={() => setIsMobileSeoToolsOpen(!isMobileSeoToolsOpen)}
              className={`flex items-center justify-between w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                isSeoToolsPage
                  ? 'bg-neutral-100 text-neutral-900'
                  : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'
            ...
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-200 ${
                  isMobileSeoToolsOpen ? 'rotate-180' : ''
                }`}
              />
            </button>
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ${
                isMobileSeoToolsOpen ? 'max-h-96 mt-1' : 'max-h-0'
              }`}
            >
              <div className="pl-3 space-y-0.5 border-l border-neutral-100 ml-3">
                {seoTools.map((tool) => (
