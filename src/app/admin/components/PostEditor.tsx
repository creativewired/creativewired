...
async function compressToWebP(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Try quality 0.85 → 0.7 → 0.5 until < 100KB
      const tryQuality = (q: number) => {
        const dataUrl = canvas.toDataURL('image/webp', q)
        const bytes = Math.round((dataUrl.length * 3) / 4)
        if (bytes < 100_000 || q <= 0.4) {
...
export default function PostEditor({ mode, initialSlug, initial }: PostEditorProps) {
  ...
  function buildMDX() {
    ...
    // Convert TipTap HTML → simple MDX-compatible markdown
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-neutral-100 p-5 space-y-4">
              ...
              <div className="border border-neutral-100 rounded-xl p-3 bg-neutral-50">
                ...
                <p className="text-[11px] text-green-700 truncate mt-0.5">creativewired.agency/blog/{slug || 'post-slug'}</p>
