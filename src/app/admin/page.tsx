...
export default async function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      {/* Top bar */}
      <header className="bg-white/80 backdrop-blur-md border-b border-neutral-200/60 px-5 md:px-8 py-4 flex items-center justify-between sticky top-0 z-40">
        <div>
          <span className="text-sm font-semibold text-neutral-900">CMS</span>
      ...
      <main className="max-w-5xl mx-auto px-5 md:px-8 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={i} className="bg-white rounded-2xl border border-neutral-200/60 p-5 shadow-sm">
          ...
          {posts.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-100">
              {posts.map((post) => (
                <div
                  key={post.slug}
                  className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-neutral-50 transition-colors"
                >
