          ...
          <div className={`grid grid-cols-2 md:grid-cols-${cs.metrics.length} divide-x divide-neutral-100`}>
            {cs.metrics.map((m, i) => (
              <div key={i} className="px-6 py-4 first:pl-0">
                <div className="text-3xl md:text-4xl font-bold text-neutral-900 tracking-tight mb-1">
                  {m.value}
                </div>
                ...
                <div className="space-y-3">
                  {cs.approach.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <span className="text-[10px] font-mono text-neutral-300 mt-1 flex-shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {prev ? (
              <Link
                ...
                className="group flex flex-col gap-2 rounded-2xl border border-neutral-100 bg-white p-6 hover:border-neutral-300 hover:shadow-sm transition-all duration-200"
              >
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-neutral-400">
                  <ArrowLeft className="w-3 h-3" />
                  Previous
                </div>
                <p className="text-sm font-semibold text-neutral-900 group-hover:text-neutral-700 transition-colors leading-snug">
                  {prev.headline}
                </p>
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border w-fit ${serviceColors[prev.service] ?? ''}`}>
                  {prev.service}
                </span>
              </Link>
            ) : <div />}
            ...
            {next ? (
              <Link
                ...
                className="group flex flex-col gap-2 rounded-2xl border border-neutral-100 bg-white p-6 hover:border-neutral-300 hover:shadow-sm transition-all duration-200 sm:text-right sm:items-end"
              >
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-neutral-400">
                  Next
                ...
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border w-fit ${serviceColors[next.service] ?? ''}`}>
                  {next.service}
                </span>
              </Link>
            ) : <div />}
